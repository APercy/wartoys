...
function wartoys_lib.rot_to_dir(rot) -- keep rot within <-pi/2,pi/2>
	local dir = minetest.yaw_to_dir(rot.y)
	dir.y = dir.y+tan(rot.x)*vector.length(dir)
	return vector.normalize(dir)
end

function wartoys_lib.dir_to_rot(v,rot)
...
function wartoys_lib.pos_shift(pos,vec) -- vec components can be omitted e.g. vec={y=1}
	vec.x=vec.x or 0
	vec.y=vec.y or 0
	vec.z=vec.z or 0
...
function wartoys_lib.get_stand_pos(thing)	-- thing can be luaentity or objectref.
	local pos = {}
	local colbox = {}
	...
	return wartoys_lib.pos_shift(pos,{y=colbox[2]+0.01}), pos
end

function wartoys_lib.get_node_pos(pos)
	return  {
			x=floor(pos.x+0.5),
			y=floor(pos.y+0.5),
			z=floor(pos.z+0.5),
...
function wartoys_lib.nodeatpos(pos)
...
function wartoys_lib.set_acceleration(thing,vec,limit)
	limit = limit or 100
	if type(thing) == 'table' then thing=thing.object end
	vec.x=wartoys_lib.minmax(vec.x,limit)
	vec.y=wartoys_lib.minmax(vec.y,limit)
	vec.z=wartoys_lib.minmax(vec.z,limit)
	
	thing:set_acceleration(vec)
end

function wartoys_lib.actfunc(self, staticdata, dtime_s)

	self.logic = self.logic or self.brainfunc
	self.physics = self.physics or wartoys_lib.physics
...
function wartoys_lib.get_box_height(self)
...
function wartoys_lib.stepfunc(self,dtime,colinfo)
	self.dtime = math.min(dtime,0.2)
	self.colinfo = colinfo
	self.height = wartoys_lib.get_box_height(self)
