function wartoys_lib.putAngleOnRange(angle)
...
function wartoys_lib.pid_controller(current_value, setpoint, last_error, delta_t, kp, ki, kd)
    kp = kp or 0
...
local function get_vehicle_pitch_roll(pos, wheels, yaw)
    ...
    for i, wheel in ipairs(wheels) do
        ...
        local height = wartoys_lib.get_obstacle({x=world.x,y=world.y,z=world.z}).y or 0
...
function wartoys_lib.ground_get_distances(self, radius, axis_distance)
...
* `minetest.raycast(pos1, pos2, objects, liquids)`: returns `Raycast`
    ...
    * `liquids`: if false, liquid nodes won't be returned. Default is `false`.
    ...
    if 1 then
        local pos_elevation = self.initial_properties.stepheight or 1.1
        local f_x, f_z = wartoys_lib.get_xz_from_hipotenuse(pos.x, pos.z, yaw, axis_distance)
...
function wartoys_lib.get_obstacle(ref_pos, ammount)
    ...
    local y = wartoys_lib.eval_interception(i_pos, {x=i_pos.x, y=i_pos.y + ammount, z=i_pos.z})
    retval.y = y
...
function wartoys_lib.get_node_below(pos, dist)
