...
wartoys_lib.modname = minetest.get_current_modname()
...
local function inventory_id(maker_name)
	local id= wartoys_lib.modname .. "_" .. maker_name .. "_"
	for i=0,5 do
...
function wartoys_lib.load_inventory(self)
	if self._inv then
...
function wartoys_lib.save_inventory(self)
    if self._inv then
...
function wartoys_lib.remove_inventory(self)
    local inventory = wartoys_lib.get_inventory(self)
    if inventory then
...
function wartoys_lib.destroy_inventory(self)
    wartoys_lib.remove_inventory(self)
    storage:set_string(self._inv_id, nil)
end

--show inventory form to user
function wartoys_lib.show_vehicle_trunk_formspec(self, player, size)
    if size then
        local form = get_formspec_by_size(self, size)
        if form then
            minetest.show_formspec(player:get_player_name(), wartoys_lib.modname .. ":inventory",
                form
            )
        end
    end
...
function wartoys_lib.create_inventory(self, size, owner)
    owner = owner or ""
    ...
    if owner ~= nil and owner ~= "" then
        ...
        wartoys_lib.load_inventory(self)
    end
end

function wartoys_lib.get_inventory(self)
    if self._inv then
...
function wartoys_lib.list_inventory(self)
    local inventory = wartoys_lib.get_inventory(self)
    if inventory then
