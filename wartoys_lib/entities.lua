core.register_entity('wartoys_lib:pivot_mesh',{
initial_properties = {
	physical = false,
	collide_with_objects=false,
	pointable=false,
	visual = "mesh",
	mesh = "wartoys_pivot_mesh.b3d",
    textures = {"wartoys_alpha.png",},
	},
...
core.register_entity('wartoys_lib:pointer',{
initial_properties = {
	physical = false,
	collide_with_objects=false,
	pointable=false,
	visual = "mesh",
	mesh = "wartoys_pointer.b3d",
    visual_size = {x = 0.5, y = 0.5, z = 0.5},
	textures = {"wartoys_white.png"},
	},
...
function wartoys_lib.on_rightclick (self, clicker)
	...
	if name == self.driver_name then
        local formspec_f = wartoys_lib.driver_formspec
        ...
        if name == self.owner or
            ...
            if clicker:get_player_control().aux1 == true then
                wartoys_lib.show_vehicle_trunk_formspec(self, clicker, self._trunk_slots)
            else
                --is the owner, okay, lets attach
                local attach_driver_f = wartoys_lib.attach_driver
            ...
            if not player_api.player_attached[name] then
                ...
                if self.driver_name then
                    local attach_pax_f = wartoys_lib.attach_pax
...
function wartoys_lib.on_punch (self, puncher, ttime, toolcaps, dir, damage)
    ...
    local speed = wartoys_lib.get_hipotenuse_value(vector.new(), velocity)
    --if math.abs(speed) <= 0.1 then
    local was_refueled = wartoys_lib.loadFuel(self, puncher:get_player_name(), false, self._max_fuel)
    ...
    if is_attached == false then
        ...
        if name == self.driver_name then
            if not self._shell_is_loaded then
                core.sound_play("wartoys_cannon_load", {
                    object = self.object,
                ...
                core.after(3, function()
                    self._shell_is_loaded = true
                    core.chat_send_player(self.driver_name, "Cannon reloaded")
                end)
            else
                local speed = 300
                local total_bullets = 30 --self._vehicle_custom_data._ww1_loaded_bullets
                local armament = "wartoys_lib:shell1"
...
function wartoys_lib.get_staticdata(self) 
...
local function scale_entity(self, scale)
    scale = scale or 1
    local initial_properties = wartoys_lib.properties_copy(self.initial_properties)
	local new_properties = wartoys_lib.properties_copy(initial_properties)
...
function wartoys_lib.on_activate(self, staticdata, dtime_s)
    if staticdata ~= "" and staticdata ~= nil then
        ...
        self._car_gravity = data.stored_car_gravity or -wartoys_lib.gravity
        self._vehicle_scale = data.stored_scale or 1
        self._vehicle_power_scale = data.stored_power_scale or 1

        wartoys_lib.setText(self, self._vehicle_name)
        if data.remove then
    ...
    if self._painting_load then
        ...
        wartoys_lib.paint(self, self._color, self._painting_textures)
    end
    ...
    if self._track_ent then
        ...
        self.rf_track = rf_track

        wartoys_lib.paint(self.lf_track:get_luaentity(), self._color, self._painting_textures)
        wartoys_lib.paint(self.rf_track:get_luaentity(), self._color, self._painting_textures)
    end

    if self._steering_ent then
	    local steering_axis=minetest.add_entity(pos,'wartoys_lib:pivot_mesh')
    ...
    wartoys_lib.seats_create(self)

    local pointer_entity = 'wartoys_lib:pointer'
	...
	if not inv then
        wartoys_lib.create_inventory(self, self._trunk_slots)
	else
	    self.inv = inv
    end

    wartoys_lib.actfunc(self, staticdata, dtime_s)
end

function wartoys_lib.on_step(self, dtime)
    wartoys_lib.stepfunc(self, dtime)
    ...
    local longit_speed = wartoys_lib.dot(velocity,hull_direction)
    local fuel_weight_factor = (5 - self._energy)/5000
    local longit_drag = vector.multiply(hull_direction,(longit_speed*longit_speed) *
        (self._LONGIT_DRAG_FACTOR - fuel_weight_factor) * -1 * wartoys_lib.sign(longit_speed))
    
	local later_speed = wartoys_lib.dot(velocity,nhdir)
    ...
    if wartoys_lib.extra_drift and longit_speed > (4*self._vehicle_power_scale) then
    ...
    if self._is_motorcycle == true then
        later_drag = vector.multiply(nhdir,later_speed*
            later_speed*self._LATER_DRAG_FACTOR*-1*wartoys_lib.sign(later_speed))
    else
        later_drag = vector.multiply(nhdir,later_speed*
            later_speed*dynamic_later_drag*-1*wartoys_lib.sign(later_speed))
    end
    ...
    if is_attached then
        ...
        if ctrl.left then pressing_left = true end
        if ctrl.right then pressing_right = true end
    end
    ...
    if self._last_light_move > 0.15 then
        ...
        if self.lights then
            if self._show_lights == true then
                --self.lights:set_properties({is_visible=true})
                self.lights:set_properties({textures={"wartoys_front_lights.png"}, glow=15})
                wartoys_lib.put_light(self)
            else
                --self.lights:set_properties({is_visible=false})
                self.lights:set_properties({textures={"wartoys_grey.png"}, glow=0})
	...
	if is_attached then --and self.driver_name == self.owner then
        local impact = wartoys_lib.get_hipotenuse_value(velocity, self.lastvelocity)
        if impact > 1 then
        ...
        self._turret_yaw = self._turret_yaw or 0
        self._turret_pitch = self._turret_pitch or 0
        
        self.object:set_bone_override("turret", {rotation = { vec = {x=0,y=self._turret_yaw,z=0}, interpolation = 5, absolute = false }})
        self.object:set_bone_override("cannon", {rotation = { vec = {x=self._turret_pitch,y=0,z=0}, interpolation = 5, absolute = false }})

        --adjust engine parameter (transmission emulation)
        local acc_factor = self._max_acc_factor
        local transmission_state = wartoys_lib.get_transmission_state(self, longit_speed, self._max_speed)
        ...
        --control
        local control = wartoys_lib.control
    ...
    --tracks
    if self.lf_track and self.rf_track and self._is_flying ~= 1 then
        self.lf_track:set_attach(self.object,'',{x=-self._track_xpos,y=0,z=0},{x=0,y=0,z=0})
        self.rf_track:set_attach(self.object,'',{x=self._track_xpos,y=0,z=0},{x=0,y=0,z=0})
    end

    --check if the tracks is touching the pavement
    local noded = wartoys_lib.nodeatpos(wartoys_lib.pos_shift(curr_pos,{y=self.initial_properties.collisionbox[2]-0.5}))

    local wheel_compensation = longit_speed * (self._wheel_compensation or 1)
    local track_framerate_L = wheel_compensation * (12 - angle_factor)
    local track_framerate_R = wheel_compensation * (12 + angle_factor)
    ...
    if (pressing_left or pressing_right) and longit_speed == 0 then
        --turn in own axis
        ...
        if pressing_left then
            angle = 2
        elseif pressing_right then
            angle = -2
        end
	    newyaw = yaw + dtime*(1 - 1 / (math.abs(fake_speed) + 1)) *
            angle * turn_rate * wartoys_lib.sign(fake_speed)

        local track_framerate = fake_speed * 8
        if pressing_left then            
        ...
        if math.abs(self._steering_angle)>5 then
	        newyaw = yaw + dtime*(1 - 1 / (math.abs(longit_speed) + 1)) *
                self._steering_angle / 30 * turn_rate * wartoys_lib.sign(longit_speed)
        end
    end

    if self.lf_track then self.lf_track:set_animation_frame_speed( track_framerate_L ) end
    if self.rf_track then self.rf_track:set_animation_frame_speed( track_framerate_R ) end
    ...
    if self._energy > 0 then
        ...
        local acceleration = wartoys_lib.get_hipotenuse_value(accel, zero_reference)
        --minetest.chat_send_all(acceleration)
        if wartoys_lib.is_drift_game == false then
    ...
    if self._energy <= 0 then
        ...
        if self._last_engine_sound_update > 0.300 then
            ...
            wartoys_lib.engine_set_sound_and_animation(self, longit_speed)
        end
    end

    local energy_indicator_angle = wartoys_lib.get_gauge_angle(self._energy)
    ...
    if not self._is_flying or self._is_flying == 0 then
        accel.y = -wartoys_lib.gravity
    else
        local time_correction = (self.dtime/wartoys_lib.ideal_step)
    ...
    if self._last_ground_check > 0.18 then
        ...
        wartoys_lib.ground_get_distances(self, 0.372*self._vehicle_scale, (self._track_length*self._vehicle_scale)/10)
    end
