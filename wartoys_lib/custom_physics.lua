function wartoys_lib.physics(self)
	...
	local spos = wartoys_lib.get_stand_pos(self)
    if not spos then return end
	spos.y = spos.y+0.01
	-- get surface height
	local snodepos = wartoys_lib.get_node_pos(spos)
	local surfnode = wartoys_lib.nodeatpos(spos)
	while surfnode and (surfnode.drawtype == 'liquid' or surfnode.drawtype == 'flowingliquid') do
		surfnodename = surfnode.name
		surface = snodepos.y +0.5
		if surface > spos.y+self.height then break end
		snodepos.y = snodepos.y+1
		surfnode = wartoys_lib.nodeatpos(snodepos)
	end
...
--		local balance = self.buoyancy*self.height
        local buoyacc = wartoys_lib.gravity*(-1)*(self.buoyancy-submergence)
    ...
    if self.isonground and not self.isinliquid then
        ...
        if not self.driver_name and math.abs(vel.x) < 0.2 and math.abs(vel.z) < 0.2 then
            self.object:set_velocity({x=0,y=wartoys_lib.gravity*(-1)*self.dtime,z=0})
